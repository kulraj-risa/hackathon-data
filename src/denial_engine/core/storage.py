...
# Firestore defaults: a DEDICATED, isolated database inside rapids-platform.
# This is intentionally NOT the (default) database and NOT the pharmacy
# product's "pharmacy" database — the hackathon app gets its own store so it
# can never read or write the real product's data. Create it once with
# scripts/setup_firestore.sh.
FIRESTORE_PROJECT_DEFAULT = "rapids-platform"
FIRESTORE_DATABASE_DEFAULT = "risa-denial-hackathon"
FIRESTORE_COLLECTION_DEFAULT = "pa_predictions"


class FirestoreStore(PredictionStore):
    """Firestore backend on its own isolated database in rapids-platform.

    Enable with:
        pip install google-cloud-firestore
        STORAGE_BACKEND=firestore
        FIRESTORE_PROJECT=rapids-platform           (default)
        FIRESTORE_DATABASE=risa-denial-hackathon     (default; the dedicated DB)
        FIRESTORE_COLLECTION=pa_predictions          (default)

    The `database` arg is what keeps this isolated from the (default) and
    `pharmacy` databases used by the real pharmacy dashboard.
    """
    ...
    def __init__(
        ...
        self.collection = collection or os.getenv(
            "FIRESTORE_COLLECTION", FIRESTORE_COLLECTION_DEFAULT
        )
        self.client = firestore.Client(
            project=project or os.getenv("FIRESTORE_PROJECT", FIRESTORE_PROJECT_DEFAULT),
            database=database or os.getenv("FIRESTORE_DATABASE", FIRESTORE_DATABASE_DEFAULT),
        )
