...
from denial_engine.ml.feature_engineer import FeatureEngineer, _num, case_fact_strings
...
class DenialPredictor:
    def __init__(self, model_path: Path = MODEL_PATH):
        ...
        # Combined feature space used by the model: numeric features first, then
        # the TF-IDF evidence vocabulary (matches model_trainer's hstack order).
        vocab = (
            [f"fact::{t}" for t in self.vectorizer.get_feature_names_out()]
            if self.vectorizer is not None
            else []
        )
        self._combined_names = list(self.feature_names) + vocab
    ...
    def predict(self, case: dict[str, Any]) -> dict[str, Any]:
        ...
        decision = self._decide(prob, level, case)
        return {
            "denial_risk": risk,
            "risk_level": level,
            ...
            "risk_factors": self._risk_factors(X),
            "model": "xgboost+tfidf" if self.vectorizer is not None else "xgboost",
            "model_auc": round(float(self.metrics.get("test_roc_auc", 0)), 3),
        }

    @staticmethod
    def _decide(prob: float, level: str, case: dict[str, Any]) -> dict[str, str]:
        """Confidence-gated routing: which cases can go touchless vs. need a human.

        A case is only eligible for touchless AUTO_SUBMIT when ALL safety
        conditions hold: low risk, a completed questionnaire, and zero
        contradictory facts. Everything risky is BLOCKed; everything in between
        goes to a human REVIEW queue. This is what lets the same team handle far
        more volume without auto-acting on cases the model hasn't earned.
        """
        nq = _num(case.get("total_questions"))
        answered = _num(case.get("answered_questions"))
        completeness = (answered / nq) if nq else 0.0
        _, con_texts, _ = case_fact_strings(case)
        con = _num(case.get("contradictory_facts")) or float(len(con_texts))

        if level == "HIGH":
            return {
                "decision": "BLOCK",
                "reason": "High denial risk — resolve the flagged gaps before submitting.",
            }
        if prob < TOUCHLESS_THRESHOLD and nq > 0 and con == 0 and completeness >= 0.9:
            return {
                "decision": "AUTO_SUBMIT",
                "reason": "Below the empirical touchless cutoff (~2% historical denial), complete and contradiction-free — eligible for touchless submission.",
            }
        return {
            "decision": "REVIEW",
            "reason": "Borderline or incomplete — route to a filer for a quick check.",
        }

    def _risk_factors(self, X: Any, top_k: int = 5) -> list[dict[str, Any]]:
        """Per-prediction explainability via XGBoost SHAP (pred_contribs).

        Returns the features pushing this case toward denial, most influential
        first, in log-odds contribution units. No external SHAP dependency.
        """
        try:
            ...
            pairs = list(zip(self._combined_names, contribs[:-1]))  # drop bias term
            pairs.sort(key=lambda p: p[1], reverse=True)
            out = []
            for name, val in pairs:
                if val <= 0 or len(out) >= top_k:
                    break
                label = _label_feature(name)
                if label is None:  # noisy TF-IDF artifact -> skip
                    continue
                out.append({"factor": label, "impact": round(float(val), 3)})
            return out
...
_FEATURE_LABELS: dict[str, str] = {
    "contradictory_facts": "Number of contradictory facts",
    "supportive_facts": "Number of supportive facts",
    "contradictory_to_supportive_ratio": "Contradictory-to-supportive ratio",
    "contradictory_minus_supportive": "Contradictions outweighing support",
    "contradictory_per_q": "Contradictions per question",
    "supportive_per_q": "Support per question",
    "has_contradictions": "Has any contradictory facts",
    "no_supportive_facts": "No supportive facts cited",
    "answer_completeness": "Questionnaire completeness",
    "unanswered_questions": "Unanswered questions",
    "answered_questions": "Answered questions",
    "total_questions": "Total questions",
    "has_questionnaire": "Questionnaire present",
    "has_med_info": "Drug class known",
    "has_payer_info": "Payer known",
}


# TF-IDF doc prefixes (ANS/SUP/CON) lowercased -> noise tokens to drop from labels.
_PREFIX_TOKENS = {"ans", "sup", "con"}


def _label_feature(name: str) -> str | None:
    """Human-readable factor label, or None if it's an uninterpretable artifact."""
    if name.startswith("fact::"):
        tokens = [t for t in name[len("fact::"):].split() if t not in _PREFIX_TOKENS]
        if not any(len(t) >= 4 and t.isalpha() for t in tokens):
            return None  # only prefix/short/numeric noise -> skip
        return f'Evidence phrase: \u201c{" ".join(tokens)}\u201d'
    if name in _FEATURE_LABELS:
        return _FEATURE_LABELS[name]
    if name.startswith("med_"):
        return f"Drug class = {name[len('med_'):]}"
    if name.startswith("payer_"):
        return f"Payer = {name[len('payer_'):].replace('_', ' ')}"
