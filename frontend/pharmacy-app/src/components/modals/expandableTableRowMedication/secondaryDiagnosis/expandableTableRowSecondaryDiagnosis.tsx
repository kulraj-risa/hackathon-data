...
import SecondaryDiagnosisCodes from "./components/secondaryDiagnosisCodes";

interface ExpandableTableRowSecondaryDiagnosisProps {
  onClose: () => void;
...
const ExpandableTableRowSecondaryDiagnosis = ({
  onClose,
  rowData,
}: ExpandableTableRowSecondaryDiagnosisProps) => {
  const [selectedTab, setSelectedTab] = useState<string>("diagnosis-codes");

  const renderContent = () => {
    switch (selectedTab) {
      case "diagnosis-codes":
        return <SecondaryDiagnosisCodes rowData={rowData} />;
  ...
  return (
    <Modal
      dialogId={ModalId.EXPANDABLE_TABLE_ROW_SECONDARY_DIAGNOSIS_MODAL}
      onSave={onClose}
      title={"Diagnosis"}
...
export default ExpandableTableRowSecondaryDiagnosis;
