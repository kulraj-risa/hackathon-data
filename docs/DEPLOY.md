...
`storage.py` is DB-agnostic. The deploy defaults to a **dedicated, isolated
Firestore database** so the live demo collects data. Options:

| Backend | How |
|---------|-----|
| Firestore (deploy default) | `STORAGE_BACKEND=firestore` → its own DB `risa-denial-hackathon` on `rapids-platform`. Run `./scripts/setup_firestore.sh` once first. |
| Local | `STORAGE_BACKEND=local` (JSON-lines file; ephemeral on Cloud Run, zero setup) |
| BigQuery | `STORAGE_BACKEND=bigquery`, set `BQ_PREDICTIONS_TABLE=project.dataset.table`, add `google-cloud-bigquery` |

### Isolated Firestore database (separate from the pharmacy product)

The hackathon app uses its **own named Firestore database** —
`risa-denial-hackathon` — inside `rapids-platform`. This is deliberately NOT
the `(default)` database and NOT the pharmacy dashboard's `pharmacy` database,
so this app can never read or write the real product's data. Isolation is
enforced by passing `database=` to the Firestore client (`storage.py`).

```bash
# One-time: create the DB + grant the Cloud Run service account access.
./scripts/setup_firestore.sh

# Deploy the API against it (this is the deploy.sh default).
STORAGE_BACKEND=firestore FIRESTORE_DATABASE=risa-denial-hackathon ./deploy.sh
```

Config knobs (env vars): `FIRESTORE_PROJECT` (default `rapids-platform`),
`FIRESTORE_DATABASE` (default `risa-denial-hackathon`), `FIRESTORE_COLLECTION`
(default `pa_predictions`).
